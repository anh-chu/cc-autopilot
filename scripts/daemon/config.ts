...
function validateConfig(config: unknown): DaemonConfig {
	...
	if (c.execution && typeof c.execution === "object") {
		...
		if (
			typeof ex.maxTaskContinuations === "number" &&
			ex.maxTaskContinuations >= 0 &&
			ex.maxTaskContinuations <= 5
		) {
	...
	if (c.inbox && typeof c.inbox === "object") {
		...
		if (
			typeof inbox.maxContinuations === "number" &&
			inbox.maxContinuations >= 0 &&
			inbox.maxContinuations <= 5
		) {
			result.inbox.maxContinuations = inbox.maxContinuations;
		}
		if (
			typeof inbox.maxTurnsPerSession === "number" &&
			inbox.maxTurnsPerSession >= 5 &&
			inbox.maxTurnsPerSession <= 100
		) {
			result.inbox.maxTurnsPerSession = inbox.maxTurnsPerSession;
		}
		if (
			typeof inbox.timeoutPerSessionMinutes === "number" &&
			inbox.timeoutPerSessionMinutes >= 5 &&
			inbox.timeoutPerSessionMinutes <= 60
		) {
...
export function loadConfig(workspaceId: string = "default"): DaemonConfig {
	...
	try {
		...
		if (config.execution.skipPermissions) {
			...
			logger.info(
				"config",
				`Allowed tools: ${config.execution.allowedTools.join(", ")}`,
			);
		}
