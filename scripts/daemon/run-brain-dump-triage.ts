...
function buildTriagePrompt(entries: BrainDumpEntry[]): string {
	...
	lines.push("");
	lines.push(
		"You are a task management assistant. Your job is to convert unprocessed brain dump entries into well-structured tasks.",
	);
	lines.push("");
	...
	if (goals.length === 0) {
		...
		for (const g of goals) {
			const proj = g.projectId
				...
				: null;
			lines.push(
				`- **${g.title}** (id: \`${g.id}\`, type: ${g.type}, status: ${g.status}${proj ? `, mission: ${proj}` : ""})`,
			);
		}
	}
	lines.push("");
	...
	lines.push("");
	lines.push(
		"2. **Create a task** by reading `~/.cmc/tasks.json`, adding a new task object to the `tasks` array, and writing it back. Task fields:",
	);
	lines.push('   - `id`: `"task_{timestamp}"` (use Date.now())');
	lines.push(
		"   - `title`: A clear, action-oriented title derived from the brain dump content",
	);
	...
	lines.push(
		"   - `projectId`: Link to a relevant mission if applicable, or `null`",
	);
	lines.push(
		"   - `milestoneId`: Link to a relevant objective if applicable, or `null`",
	);
	lines.push(
		'   - `assignedTo`: The most suitable AI agent ID, or `"me"` for human tasks',
	);
	lines.push("   - `collaborators`: `[]`");
	lines.push(
		"   - `subtasks`: Break into sub-steps if the task is complex, otherwise `[]`",
	);
	...
	lines.push("");
	lines.push(
		"3. **Mark the brain dump entry as processed** by reading `~/.cmc/brain-dump.json`, finding the entry by ID, setting `processed` to `true` and `convertedTo` to the task ID you created, and writing the file back.",
	);
	lines.push("");
	...
	lines.push(
		"- Do NOT delete or modify existing entries — only add new tasks and update the `processed`/`convertedTo` fields on brain dump entries.",
	);
	lines.push(
		"- You MUST complete both step 2 (create task) AND step 3 (mark entry processed) for each entry.",
	);
...
async function main() {
	...
	logger.info(
		"brain-dump-triage",
		`Starting auto-triage for ${entryIds.length} entries: ${entryIds.join(", ")}`,
	);
	...
	if (entries.length === 0) {
		logger.error(
			"brain-dump-triage",
			"No unprocessed entries found matching the provided IDs",
		);
		process.exit(1);
	}
	...
	try {
		...
		if (result.exitCode === 0) {
			logger.info(
				"brain-dump-triage",
				`Auto-triage completed for ${entries.length} entries`,
			);
		} else {
			logger.error(
				"brain-dump-triage",
