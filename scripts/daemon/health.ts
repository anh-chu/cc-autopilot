...
export class HealthMonitor {
	...
	endSession(
		...
		const durationMinutes =
			Math.round(((Date.now() - startTime) / 60_000) * 100) / 100;
		...
		if (usage) {
			...
			this.stats.totalCacheCreationTokens +=
		...
		if (status === "completed") {
			...
			logger.info(
				"health",
				`Session completed: ${sessionId} (${durationMinutes}min${costStr})`,
			);
		} else {
	...
	getRetryCount(taskId: string): number {
		...
		return this.history.filter(
			(h) => h.taskId === taskId && h.status !== "completed",
		).length;
	}
		...
		for (const [id, session] of this.activeSessions) {
			...
			if (!isProcessRunning(session.pid)) {
				...
				this.endSession(
					id,
					1,
					"Process died unexpectedly (detected by health check)",
					false,
				);
			}
		}
	...
	private loadPersistedStats(): void {
		try {
			...
			if (data.stats) {
				...
				this.stats.totalCacheCreationTokens =
					data.stats.totalCacheCreationTokens || 0;
			}
			if (data.history) {
