...
function reconcileStuckMissions(missions: MissionEntry[]): boolean {
	...
	for (const mission of missions) {
		...
		if (remaining.length === 0) {
			...
			mission.completedTasks = projectTasks.filter(
				(t) => t.kanban === "done",
			).length;
			changed = true;
...
function postMissionInboxReport(mission: MissionEntry): void {
	try {
		...
		if (isComplete) {
			lines.push(
				`All ${mission.completedTasks} tasks in this project run have been completed.`,
			);
		} else {
			lines.push(
				`The project run has stalled with ${remaining} task(s) remaining that could not be dispatched.`,
			);
		}
...
export async function GET() {
	try {
		...
		const data = JSON.parse(readFileSync(missionsPath, "utf-8")) as {
