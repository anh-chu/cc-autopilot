...
async function addInboxMessage(
	msg: Omit<InboxMessage, "id" | "createdAt" | "readAt" | "status">,
) {
...
async function handleDelegation(
	task: Task,
	previousAssignee: AgentRole | null,
) {
...
async function handleCollaboratorChanges(
	task: Task,
	previousCollaborators: string[],
) {
	const currentCollabs = task.collaborators ?? [];
	const newCollabs = currentCollabs.filter(
		(c) => !previousCollaborators.includes(c),
	);
	const removedCollabs = previousCollaborators.filter(
		(c) => !currentCollabs.includes(c),
	);
	...
	if (newCollabs.length > 0 || removedCollabs.length > 0) {
		...
		if (removedCollabs.length > 0)
			parts.push(`removed: ${removedCollabs.join(", ")}`);
		await addActivityEvent({
			type: "task_updated",
			actor: "system",
			taskId: task.id,
