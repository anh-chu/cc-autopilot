...
export async function POST(
	...
	if (task.comments.length >= 100) {
		return NextResponse.json(
			{ error: "Max 100 comments per task" },
			{ status: 400 },
		);
	}
	...
	const agentsData = readJSON<{ agents: AgentEntry[] }>(
		path.join(wsDir, "agents.json"),
	);
	...
	try {
		...
		const activityRaw = existsSync(activityPath)
			...
			: '{"events":[]}';
		const activityData = JSON.parse(activityRaw) as {
	...
	if (Array.isArray(deletedComment.attachments)) {
		for (const att of deletedComment.attachments) {
			if (typeof att.url === "string" && att.url.startsWith("/uploads/")) {
				try {
					unlinkSync(
						path.join(getUploadsDir(workspaceId), path.basename(att.url)),
					);
				} catch {
					/* ignore */
				}
