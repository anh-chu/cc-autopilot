...
function safeWikiPath(wikiDir: string, rel: string): string | null {
  ...
  if (resolved !== wikiDir && !resolved.startsWith(wikiDir + path.sep)) return null;
  return resolved;
}
...
export async function POST(request: Request) {
  ...
  if (!targetDir) return NextResponse.json({ error: "Invalid directory" }, { status: 400 });
  ...
  return NextResponse.json({ filename: savedFilename, path: relPath, originalName: file.name });
}
