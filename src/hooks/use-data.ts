	...
	const bulkUpdate = useCallback(
		async (ids: string[], updates: Partial<T>) => {
			...
			try {
				...
				showSuccess(
					`${ids.length} ${label.toLowerCase()}${ids.length > 1 ? "s" : ""} updated`,
				);
			} catch (err) {
	...
	const bulkRemove = useCallback(
		async (ids: string[]) => {
			...
			try {
				...
				showSuccess(
					`${ids.length} ${label.toLowerCase()}${ids.length > 1 ? "s" : ""} deleted`,
				);
			} catch (err) {
...
export function useTasks() {
	const { items: tasks, ...rest } = useDataResource<Task>(
		"tasks",
		"tasks",
		"Task",
		15_000,
	...
	const refetch = useCallback(async () => {
		try {
			...
			const res = await apiFetch(
				`/api/tasks?initiativeId=${encodeURIComponent(initiativeId)}`,
			);
...
export function useAgents() {
	const { items: agents, ...rest } = useDataResource<AgentDefinition>(
		"agents",
		"agents",
		"Agent",
	);
...
export function useInitiatives() {
	const { items: initiatives, ...rest } = useDataResource<Initiative>(
		"initiatives",
		"initiatives",
		"Initiative",
	);
