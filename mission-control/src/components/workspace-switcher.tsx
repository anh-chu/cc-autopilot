...
export function WorkspaceSwitcher({ collapsed = false }: WorkspaceSwitcherProps) {
  const { workspaces, currentWorkspace, currentId, loading, switchWorkspace, createWorkspace } =
    useWorkspace();
  ...
  if (loading) {
    return (
      <div className={cn(
        ...
        {!collapsed && <div className="h-3 w-20 rounded bg-sidebar-foreground/10" />}
      </div>
    );
  }

  const display = currentWorkspace ?? { name: "Default", color: "#6366f1", id: "default" };
        ...
        <DropdownMenuContent side="right" align="start" className="w-52">
          {workspaces.map((ws) => (
            ...
            >
              ...
              {ws.id === currentId && <Check className="h-3.5 w-3.5 text-primary" />}
            </DropdownMenuItem>
          ))}
