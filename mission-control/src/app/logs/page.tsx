...
export default function LogsPage() {
  ...
  useEffect(() => {
    ...
    async function fetchLog(
      ...
      try {
        ...
        if (!response.ok) {
          if (!isMounted) return;
          setLog((current) => ({ ...current, error: `Failed to load ${label}.`, isLoading: false }));
          return;
        }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Live daemon output, active runs, and quick access to per-run consoles.
          </p>
        </div>
      ...
      {(showDaemon || showApp) && (
        <div className={`grid gap-6 ${showDaemon && showApp ? "xl:grid-cols-2" : ""}`}>
          {showDaemon && (
            <LogTailCard
              title="Daemon Log Tail"
              description={live ? "Streaming live. New lines appear in real-time." : "Last 50 lines. Refreshes every 5 seconds."}
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Current background work from the active runs registry.</CardDescription>
            </CardHeader>
            <CardContent>
              {filteredRuns.length === 0 ? (
                <p className="text-sm text-muted-foreground">No active or recent runs found.</p>
              ) : (
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Badge variant="secondary" className={getStatusBadgeClass(run.status)}>
                            {run.status}
                          </Badge>
                          {run.taskId ? (
                            <Badge variant="outline" className="text-xs">
                        ...
                        <p className="text-xs text-muted-foreground break-all">Run ID: {run.id}</p>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Bounded error excerpts for failed or timed out runs.</CardDescription>
            </CardHeader>
            <CardContent>
              {failedRuns.length === 0 ? (
                <p className="text-sm text-muted-foreground">No failed runs recorded.</p>
              ) : (
                <div className="space-y-3">
                  {failedRuns.map((run) => (
                    <div key={run.id} className="rounded-lg border border-red-500/20 bg-red-500/5 p-4">
                      ...
                      <p className="mb-2 text-xs text-muted-foreground">
                        {run.taskId ? `Task ${run.taskId}` : "Unlinked run"} · Started{" "}
          ...
          <CardHeader>
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <CardDescription>Live stream for run {selectedRunId}</CardDescription>
              </div>
