...
export async function GET(request: NextRequest) {
  ...
  if (!isAllowedLogPath(logPath)) {
    return NextResponse.json({ lines: [], error: "Forbidden" }, { status: 403 });
  }

  const rawLines = Number.parseInt(request.nextUrl.searchParams.get("lines") ?? "", 10);
  ...
  try {
    ...
    return NextResponse.json({ lines: [], error: "Failed to read daemon log" }, { status: 500 });
  }
}
