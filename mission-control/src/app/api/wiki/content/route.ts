...
const TEXT_EXTS = new Set(["txt", "md", "markdown", "json", "yaml", "yml", "toml", "csv", "xml", "html", "css", "js", "ts", "sh"]);
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (buffer.length > MAX_EDIT_SIZE) {
      return NextResponse.json({ error: "File too large to edit (max 1MB)" }, { status: 413 });
    }
