      ...
      <div className="flex-1 p-6 space-y-6 max-w-2xl">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Configure your workspace name, appearance, and defaults.</CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Background task execution, polling intervals, and concurrency limits.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-5">
            <div className="flex items-center gap-3">
              ...
              {isRunning ? (
                <Button size="sm" variant="outline" className="h-7 gap-1.5" onClick={() => void stop()}>
                  <Square className="h-3 w-3" /> Stop
                </Button>
              ) : (
                <Button size="sm" className="h-7 gap-1.5" onClick={() => void start()}>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Automatically pick up new tasks</p>
              </div>
              <Switch
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={e => setMaxParallelAgents(Math.max(1, Math.min(10, Number(e.target.value))))}
                className="max-w-[120px]"
              />
            </div>

            <div className="flex items-center gap-3">
              <Button onClick={() => void handleDaemonSave()} disabled={daemonSaving}>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>Save and restore workspace snapshots.</CardDescription>
              </div>
              <div className="flex items-center gap-2">
                ...
                <Button
                  size="sm"
                  className="gap-1.5 h-7 text-xs"
                  onClick={() => { setSaveName(""); setSaveDesc(""); setShowSave(true); }}
                  disabled={cpBusy}
                >
          ...
          <CardContent>
            {cpLoading ? (
              ...
              <p className="text-sm text-muted-foreground py-2">
                No checkpoints yet. Create a backup to snapshot your current workspace.
              </p>
            ) : (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        {cp.stats.projects > 0 && ` · ${cp.stats.projects} projects`}
                      </p>
                    </div>
      ...
      <Dialog open={showSave} onOpenChange={setShowSave}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCpSave} disabled={!saveName.trim() || cpBusy}>
              {cpBusy ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Save className="h-4 w-4 mr-2" />}
              Save Checkpoint
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              Make sure to create a backup first if you want to keep your current state.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowLoad(null)}>Cancel</Button>
            <Button onClick={handleCpLoad} disabled={cpBusy}>
              {cpBusy ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <FolderOpen className="h-4 w-4 mr-2" />}
              Restore
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={!!showDelete} onOpenChange={open => !open && setShowDelete(null)}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleCpDelete} disabled={cpBusy}>
              {cpBusy ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Trash2 className="h-4 w-4 mr-2" />}
              Delete
            </Button>
          </DialogFooter>
        </DialogContent>
