...
function sanitizeMeta(meta: unknown): unknown {
  ...
  if (typeof meta === "object") {
    const entries = Object.entries(meta as Record<string, unknown>).map(([key, value]) => [
      key,
      sanitizeMeta(value),
    ]);
...
export function createLogger(processName: string, opts: LoggerOptions = {}): Logger {
  ...
  const write = (level: LogLevel, module: string, message: string, meta?: unknown): void => {
