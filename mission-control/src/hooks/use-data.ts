...
export function useInitiativeTasks(initiativeId: string) {
  ...
  const refetch = useCallback(async () => {
    try {
      if (!initialLoadDone.current) setLoading(true);
      const res = await apiFetch(`/api/tasks?initiativeId=${encodeURIComponent(initiativeId)}`);
...
export function useInitiatives() {
  const { items: initiatives, ...rest } = useDataResource<Initiative>("initiatives", "initiatives", "Initiative");
