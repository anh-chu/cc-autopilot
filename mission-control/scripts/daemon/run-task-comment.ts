...
function buildCommentPrompt(agent: AgentDef, task: TaskDef, comment: string, commentAuthor: string): string {
  const lines: string[] = [];
  ...
  lines.push("If the message is a question, request for info, or feedback that doesn't require code changes,");
  lines.push("simply write your reply. The system will capture your response and add it as a comment on the task.");
  lines.push("");
  lines.push("**Option B — Reopen and work on the task:**");
  lines.push(`If the task needs rework (even though its current status is "${task.kanban}"), you should:`);
  ...
  lines.push("- If you made file changes, update the task status appropriately");
  lines.push("");
  lines.push("**IMPORTANT:** Focus on responding to the specific message. Be concise and helpful.");
  lines.push("Do NOT modify tasks.json, inbox.json, or activity-log.json directly.");
...
function appendAgentComment(taskId: string, agentId: string, response: string): void {
  try {
    ...
    logger.info("run-task-comment", `Appended agent response comment to task ${taskId}`);
  } catch (err) {
    logger.error("run-task-comment", `Failed to append comment: ${err instanceof Error ? err.message : String(err)}`);
  }
}

function postInboxNotification(taskId: string, agentId: string, taskTitle: string, response: string): void {
  try {
    ...
    logger.error("run-task-comment", `Failed to post inbox notification: ${err instanceof Error ? err.message : String(err)}`);
  }
}
...
function logActivity(taskId: string, agentId: string, summary: string): void {
  try {
    ...
    logger.error("run-task-comment", `Failed to log activity: ${err instanceof Error ? err.message : String(err)}`);
  }
}
...
function parseArgs(): {
  ...
  if (!taskId) {
    console.error("Usage: run-task-comment.ts <taskId> --agent <agentId> --comment <text> --comment-author <author>");
    process.exit(1);
  }
...
async function main() {
  ...
  // 1. Read task
  const tasksData = readTasks();
  const task = tasksData.tasks.find((t) => t.id === taskId) as TaskDef | undefined;
  if (!task) {
  ...
  try {
      ...
      if (didWork) {
        try {
          ...
          if (freshTask && freshTask.kanban === "done") {
            ...
            writeFileSync(TASKS_FILE, JSON.stringify(freshTasks, null, 2), "utf-8");
            logger.info("run-task-comment", `Reopened task ${taskId} (agent did substantive work)`);
          }
        } catch (err) {
          logger.error("run-task-comment", `Failed to reopen task: ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    ...
    logger.info("run-task-comment", `Comment handler complete for task ${taskId} (agent: ${agentId}, turns: ${meta.numTurns ?? "?"}${costStr})`);
...
main().catch((err) => {
  logger.error("run-task-comment", `Unhandled error: ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});
